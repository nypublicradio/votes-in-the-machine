...
class APClient {
  ...
  async nextRequest() {
    ...
    console.log('getting next request value');
    try {
      let { nextrequest } = await s3.getObject(ops).promise();
      return nextrequest;
    } catch(e) {
      if (e.code === 'NoSuchKey') {
        return null;
  ...
  async fetchResults() {
    ...
    console.log('calling ap endpoint');
  ...
  async fetchCachedResults() {
    ...
    return s3.getObject(getOps).promise();
  }
  ...
  cacheNextRequest(nextrequest) {
    ...
    console.log('caching next request', uploadOptions);
  ...
  mungeRace(race) {
    ...
    // stateLevelInfo.candidates.forEach(c => c.voteCount = Math.round(Math.random() * 5000));
    // stateLevelInfo.candidates[Math.round(Math.random() * (stateLevelInfo.candidates.length - 1))].winner = 'X';
    ...
    return {
      ...
      candidates: stateLevelInfo.candidates.sort(byVotesByName),
    };
  }
}
