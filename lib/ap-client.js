...
class APClient {
  ...
  async mergeResults(nexturl) {
    ...
    let now = moment();
    currentResults.lastUpdated = now.format();
    currentResults.pollsClosed = now.diff(moment(this.pollsCloseAt, 'YYYY-MM-DD-HH-mm'), 'minutes') >= 0;
  ...
  mungeRace(race) {
    ...
    if (race.reportingUnits) {
      ...
      let { precinctsReporting, precinctsTotal, statePostal, stateName } = stateLevelInfo;
      ...
      // zero out the results JIC
      if (race.candidates.length) {
        race.candidates.forEach(c => c.voteCount = c.voteCount || 0);
      }

      results.candidates = race.candidates;
      results.statePostal = race.statePostal;
    }

    const ordered = {};
    Object.keys(results).sort().forEach(key => ordered[key] = results[key]);
    return ordered;
  }
}
